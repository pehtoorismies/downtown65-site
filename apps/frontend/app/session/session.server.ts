...
import { commitMessageSession, getMessageSession, setMessage } from '~/message.server'
...
const createSessionStorage = ({ ENVIRONMENT, COOKIE_SECRET }: SessionsParams) => {
  return createCookieSessionStorage({
    cookie: {
      ...
      path: '/',
      sameSite: 'lax',
      secrets: [COOKIE_SECRET], // Access from env
...
export const getSession = (request: Request) => {
  ...
  const params: SessionsParams = {
    ...
    COOKIE_SECRET: '1234567890abcdef1234567890abcdef', // Access from env
  }
  const sessionStorage = createSessionStorage(params)
...
export const getDestroySessionHeaders = async (request: Request, message: ToastMessage) => {
...
const getAuthentication = async (request: Request): Promise<AuthenticationResponse | undefined> => {
  const session = await getSession(request)
  ...
  const { user, headers, accessToken } = await renewSession(sessionData, session, request)
...
export const getAuthenticatedUser = async (request: Request): Promise<User | undefined> => {
...
export const actionAuthenticate = async (request: Request): Promise<AuthenticationResponse> => {
  const result = await getAuthentication(request)
  if (!result) {
    const headers = await getDestroySessionHeaders(request, {
      message: 'Sessio vanhentunut',
